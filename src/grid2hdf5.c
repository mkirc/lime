...
from an HDF5 format file. The purpose of the present comment block is to describe the
HDF5 file format. Note that the amount and type of information stored depends on the
...
second row for column descriptions indicates the HDF5 data type according to the
following key:
...
0   The file.
Attributes:
0   RADIUS             D	# The model radius in m.
0   MINSCALE           D	# The model minimum scale in m.
...
1   Group 'LINKS'
(see explanation in the header to gridio.c)

1   Group 'LINKS/columns'
Number of elements in each of the datasets = number of Delaunay links.

Datasets ('GRID/LINKS/columns/'):
1    GRID_I_1   V
1    GRID_I_2   V
7    V_p_j      D	# 1 per pth velocity sample per jth dimension.

8   Group 'LEVEL_POPS_m'
(1 per mth molecular species)
Attributes:
8    MOL_NAME           A
8    Dataset 'array'    E
'array' has shape (number of energy levels this species),(number of grid cells)
...
Datasets:
1    LINK_I     V
'LINK_I' has shape (number of grid points * average number of Delaunay links per point)
